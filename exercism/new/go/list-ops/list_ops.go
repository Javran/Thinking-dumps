// Package listops contains functional list operations operating on []int.
// Note that all of those functions are functional, meaning they keep the receiver intact.
package listops

// IntList is a type synonym for slices of int.
type IntList []int
...
// Append appends another slice to the existing slice.
...
// Concat appends multiple slices to the existing slice.
...
// Foldl performs left-fold through a IntList and accumulates its result.
...
// Foldr performs right-folds through a IntList and accumulates its result.
...
// Filter returns a slice that only contains elements satisfying a given predicate.
...
// Map applies a function to every element of the slice and returns the resulting slice.
