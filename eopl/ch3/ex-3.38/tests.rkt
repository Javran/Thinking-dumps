...
in let times4 = (fix t4m)
   in (times4 3)" 12)
      ...
      ;; check cond
      (test-cond-1 "let x = 1 in let y = 1 in cond zero?(-(x,y)) ==> 100 zero?(1) ==> 200 end" 100)
      (test-cond-2 "cond zero?(1) ==> 1 zero?(2) ==> 2 zero?(0) ==> 3 end" 3)
      (test-cond-3 "cond zero?(1) ==> 1 zero?(2) ==> 2 zero?(3) ==> 3 end" error)
