    ...
    (lambda (exp nameless-env)
      (cases expression exp
        ...
        (cond-exp (exps1 exps2)
          (define (travel-cases preds conseqs)
            (if (null? preds)
              ; all cases failed
              (eopl:error 'value-of
                "cond-exp has a uncaught case")
              ; evaluate next predicate
              (let ((result (value-of (car preds) nameless-env)))
                (if (expval->bool result)
                  ; predicate returns true, apply the procedure
                  (value-of (car conseqs) nameless-env)
                  ; else keep going
                  (travel-cases (cdr preds)
                                (cdr conseqs))))))
          (travel-cases exps1 exps2))
