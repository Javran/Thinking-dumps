...
"letrec even(odd)  = proc(x) if zero?(x) then 1 else (odd -(x,1))
   ...
   in (odd 13)" 1)
      ...
      ;; test multiple argument support
      (test-multi-arg "(proc (a b c) -(a,-(b,c)) 10 20 30)" 20)
      (test-multi-arg-let-rec-1
        "letrec fib (n a b) =
          if zero?(n)
            then b
            else (fib -(n,1) b -(a,-(0,b)))
         in (fib 10 0 1)" 89)
      ...
      ;; test multiple arguments + muturally recursive
      (test-multi-and-mutual
        "letrec fib0 (n a b) = if zero?(n) then b else (fib1 -(n,1) b -(a,-(0,b)))
         ...
         in -((fib0 10 0 1),-((fib1 7 0 1),(fib2 4 0 1)))"
         73)
      ; 89-(21-5) =73
