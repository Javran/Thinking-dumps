...
in begin
    ...
    z
      ...
      ; `(arrayref a (arrayref a i))` might cause an error
      ;   because (arrayref a i) will be evaluated to an expval,
      ;   which is never guaranteed to be a valid index of array `a` 
      ;   in the following testcase, `(arrayref a 0)` evaluates to `3`,
      ;   which is not a valid index of `a`.
      (array-swap-test-in-book "
        let swap =
              proc (x)
                proc (y)
                  let temp = x
        ...
        in begin
             arrayset(arr,0,3);
             % here
             ((swap arrayref(arr,arrayref(arr,0))) arrayref(arr,1));
             -(arrayref(arr,0),arrayref(arr,1))
           end" error)
