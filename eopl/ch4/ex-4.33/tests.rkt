...
in begin
    ...
    z
      ...
      (cbv-global-aliasing-1
        "let p = procv (z) set z = 44
         in let x = 33
         in begin (p x); x end" 33)

      (cbv-direct-aliasing-1
        "let p =
          procv (x)
            procv (y)
              begin
      ...
      (cbv-indirect-aliasing-1
        "let p = procv (x) procv (y)
                   begin
                    set x = 44;
                    y
                   end
      ...
      ; our copy is not deep enough
      (cbv-sideeffect-a-passed-structure-1
        "let f = procv (x)
                   setleft x = -(left(x),-1)
         in let p = newpair (44,newpair(55,66))
         in begin
      ...
      (cbv-example-for-book "
        let f = procv (x) set x = 44
        in let g = procv (y) (f y)
        in let z = 55
        in begin
             (g z);
             z
