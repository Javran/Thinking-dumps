...
-1)
      ...
      ;; duplicate some testcases from above and adjust them
      ;;   to test the correctness of `procr`
      ;; (respecting the rule that the argument
      ;;   is required to be a variable)
      (procv-correctness-1 "
         let a = 30 in
         (procr(x) -(x,1) a)" 29)
      (procv-correctness-2 "
         let a = 30 in
         let f = procr (x) -(x,1) in 
         (f a)" 29)
      (procv-correctness-3 "
         let a = 30 in
         let b = procr(x) -(x,1) in
         (procr(f)(f a) b)" 29)
      (provc-correctness-4 "
         let a = 5 in
         let b = 6 in
         ((procr (x) procr (y) -(x,y)  a) b)" -1)
      (provc-correctness-5 "
         let a = -(10,5) in
         let b = 6 in
         let f = procr(x) procr (y) -(x,y) in
         ((f a) b)" -1)
      
      ; for call-by-reference strategy,
      ;   the program yields 10
      (different-result-1 "
        let a = 1 in
        let f =
          proc (x)
      ...
      ; for call-by-result-value strategy,
      ;   the program yields 1
      (different-result-2 "
        let a = 1 in
        let f =
          procr (x)
