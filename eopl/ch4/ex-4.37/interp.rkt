  ...
  (define value-of
    (lambda (exp env)
	  ...
	  (when (instrument-let)
        ...
        (proc-exp (var body)
          (proc-val
            (procedure var body env #f)))

        (procr-exp (var body)
          (proc-val
            (procedure var body env #t)))

        (call-exp (rator rand)
          (let ((proc1 (expval->proc (value-of rator env)))
                (arg (value-of-operand rand env)))
            (cases proc proc1
              (procedure (var body saved-env force-var)
                (if force-var
                  ; call-by-result-value proc
                  (cases expression rand
                    (var-exp (v)
                      (apply-procedure proc1 arg))
                    (else
                      (eopl:error 'call-exp
                        "The actual parameter can only be a variable")))
                  ; call-by-reference proc
                  (apply-procedure proc1 arg))))))
    ...
    (lambda (proc1 val)
      (cases proc proc1
        (procedure (var body saved-env force-var)
          (if force-var
            ; procv
            ; * new-val: shallow copy the value to a new place
            ; * result: execute the body normal, bind the return value to `result`
            (let* ((new-val (newref (deref val)))
                   (new-env (extend-env var new-val saved-env))
                   (result (value-of body new-env)))
              (begin
                ; write back (shallow, of course)
                (setref! val (deref new-val))
                result))
            ; proc
