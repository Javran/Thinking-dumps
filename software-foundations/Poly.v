...
I don't know what is the meaning of asking how many elements
does a type have. But I guess it's 0.

Justification: there are two "type constructors", but all of them
needs to feed in a value of type "baz", which is impossible at the first place.
...
Proof.
  ...
  Case "s = cons n s'". simpl. rewrite IHs'. reflexivity.
Qed.
...
Proof.
  ...
  Case "l = n l'". simpl. rewrite rev_snoc. rewrite IHl'.
  reflexivity.
Qed.
...
Proof.
  intros X l1 l2 v. induction l1 as [|n1 l1'].
  Case "l1 = nil". reflexivity.
  Case "l1 = n1 l1'". simpl. rewrite IHl1'. reflexivity.
Qed.
(** [] *)
...
(** Try answering the following questions on paper and
  ...
  1. forall (X Y : Type), list X -> list Y -> list (X*Y)
  2. [(1,false); (2,false)]

*)
Check @combine.

Example combine_check_1 :
...
(*
  - prod_curry : forall X Y Z, ((X * Y) -> Z) -> X -> Y -> Z
...
Example test_filter_even_gt7_my :
  filter_even_gt7 [1;2;3;4;5;6;7;8;9] = [8].
Proof. reflexivity. Qed.
(** [] *)
...
Lemma rev_cons : forall (X : Type) (v : X) (s : list X),
  rev (cons v s) = snoc (rev s) v.
Proof.
  intros X v s. induction s as [|u s'].
  Case "s = nil". reflexivity.
  Case "s = cons u s'". rewrite <- rev_involutive. simpl.
    rewrite rev_involutive. reflexivity.
Qed.

Lemma map_cons : forall (X Y : Type) (f : X -> Y) (x : X) (xs : list X),
  map f (x :: xs) = f x :: map f xs.
Proof.
  intros X Y f x xs. reflexivity.
Qed.

Lemma map_snoc : forall (X Y : Type) (f : X -> Y) (x : X) (xs : list X),
  map f (snoc xs x) = snoc (map f xs) (f x).
Proof.
  intros X Y f x xs. induction xs as [|x' xs'].
  reflexivity. simpl. rewrite <- IHxs'. reflexivity.
Qed.
...
Proof.
  ...
  Case "l = nil". reflexivity.
  Case "l = cons x l'". rewrite map_cons. rewrite rev_cons.
    rewrite map_snoc. simpl. rewrite IHl'. reflexivity.
Qed.
(** [] *)
...
(*
   to reverse a list [xs], you can use:
   [fold (fun i acc => acc ++ [i]) [] xs]
   here [X = X'], [Y = [X']]
*)
...
(* it overrides the return value to true when [3] is given,
   but here the resulting function is given [2], which is
   still the same as [constfun b]. *)
...
Proof.
  ...
  Case "l = nil". reflexivity.
  Case "l = cons x l'". simpl. rewrite <- IHl'.
...
Definition fold_map {X Y:Type} (f : X -> Y) (l : list X) : list Y :=
  fold (fun i acc => f i :: acc) l [].

(** Write down a theorem in Coq stating that [fold_map] is correct,
    and prove it. *)
Theorem fold_map_correct : forall (X Y : Type) (f : X -> Y) (xs : list X),
  fold_map f xs = map f xs.
Proof.
  intros X Y f xs. induction xs as [|x' xs'].
  Case "xs = nil". reflexivity.
  Case "xs = cons x' xs'". simpl. rewrite <- IHxs'.
